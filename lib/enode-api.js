...
class EnodeAPI {
  ...
  async getAccessToken(vehicleId = null, clientId = null) {
    try {
      ...
      try {
        ...
        // Fallback to AccountManager for backward compatibility
    ...
    try {
      ...
      if (!clientId) {
        try {
          ...
          this.api.log(`Using default client for link generation: ${clientId}`);
        } catch (error) {
      ...
      if (getAllClients) {
        ...
        try {
          ...
          if (clients.length === 0) {
            ...
            return this._getVehiclesUsingAccountManager();
          }
                ...
                clientVehicles.forEach(vehicle => {
                  if (vehicle.information && vehicle.information.vin) {
                    try {
                      ...
                      this.api.error(`Error associating vehicle ${vehicle.information.vin} with client ${client.id}:`, error);
                    }
                  }
                });
   ...
   * @private
   */
  async _getVehiclesUsingAccountManager() {
