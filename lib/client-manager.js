...
class ClientManager {
  ...
  _migrateAndSetDefaultClient() {
    try {
      ...
      // Add additional clients if they exist in Homey.env (client_3 to client_10)
      for (let i = 3; i <= 10; i++) {
        const clientIdKey = `ENODE_CLIENT_ID_${i}`;
        const clientSecretKey = `ENODE_CLIENT_SECRET_${i}`;

        // Check if the keys exist in Homey.env before accessing to avoid warnings
        if (Homey.env.hasOwnProperty(clientIdKey) && Homey.env.hasOwnProperty(clientSecretKey)) {
