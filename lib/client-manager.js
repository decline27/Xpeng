...
class ClientManager {
  ...
  _migrateAndSetDefaultClient() {
    try {
      ...
      if (secondaryClientId && secondaryClientSecret) {
        ...
        // Check if we should clear the legacy secondary default to enable sequential distribution
        const currentDefault = this.homey.settings.get(this.SETTINGS_KEYS.DEFAULT_CLIENT);
        if (currentDefault === this.LEGACY_CLIENTS.SECONDARY) {
          this.logger('Detected legacy secondary default, clearing to enable sequential distribution');
          this.homey.settings.set(this.SETTINGS_KEYS.DEFAULT_CLIENT, null);
        }
      } else if (primaryClientId && primaryClientSecret) {
        // If no secondary client, set primary as default (as a safe baseline)
        const currentDefault = this.homey.settings.get(this.SETTINGS_KEYS.DEFAULT_CLIENT);
        if (!currentDefault) {
      ...
      // Add additional clients if they exist in Homey.env (client_3 onwards)
      // Only access keys that exist to avoid Homey.env access warnings
  ...
  /**
   * Remove stale clients from the registry that are no longer configured
   * @param {Array<string>} validClientIds - List of currently valid client IDs
   */
  removeStaleClients(validClientIds) {
    const registry = this.homey.settings.get(this.SETTINGS_KEYS.CLIENT_REGISTRY) || { clients: [] };
    const before = registry.clients.length;
    registry.clients = registry.clients.filter(c => validClientIds.includes(c.id));
    const removed = before - registry.clients.length;
    if (removed > 0) {
      this.homey.settings.set(this.SETTINGS_KEYS.CLIENT_REGISTRY, registry);
      this.logger(`Removed ${removed} stale client(s) from registry`);
    }
  }
