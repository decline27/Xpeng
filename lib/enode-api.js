...
class EnodeAPI {
  ...
  async getVehicles() {
    try {
      ...
      // ENHANCED SECURITY: Filter vehicles by authorized VINs and unique user ID
      try {
        ...
        if (allAuthorizedVins.length > 0) {
          ...
          const filteredVehicles = responseData.data.filter(vehicle =>
          ...
          if (filteredVehicles.length > 0) {
            ...
            // Cache the filtered response with 1-minute TTL
            this.requestCache.set(cacheKey, filteredVehicles, 60000);
        ...
        const userVehicles = responseData.data.filter(vehicle =>
        ...
        if (userVehicles.length > 0) {
          ...
          if (authorizedVins.length > 0) {
            ...
            // Clear the vehicles cache to ensure we get fresh data next time
            this.requestCache.clear(cacheKey);
          }

          // Cache the filtered response with 1-minute TTL
          this.requestCache.set(cacheKey, userVehicles, 60000);
          return userVehicles;
        }

        // Only during pairing mode, return all vehicles
        if (isPairingMode) {
          this.api.log('In pairing mode, returning all vehicles for selection');
          return responseData.data;
        }

        // If we get here and we're not in pairing mode, return an empty array
        // This is safer than returning all vehicles
        this.api.log('No vehicles found matching filters and not in pairing mode. Returning empty list for security.');
        return [];
      } catch (filterError) {
        this.api.error('Error filtering vehicles:', filterError);
        // For security, return empty array on error unless in pairing mode
        const isPairingMode = new Error().stack.includes('onPair') ||
                             new Error().stack.includes('list_devices');
        if (isPairingMode) {
          return responseData.data;
