...
class EnodeAPI {
  ...
  async refreshVehicleData(clientId, clientSecret, vehicleId, waitMs = 0) {
    ...
    try {
      ...
      const lastRefresh = this.requestCache.get(lastRefreshKey) || 0;
      ...
      if (now - lastRefresh > 60 * 60 * 1000) {
        ...
        throw new Error('Rate limit reached for refresh-hints');
      }
      ...
      if (error.message.includes('429')) {
        this.api.log(`Rate limit hit for vehicle ${vehicleId}`);
