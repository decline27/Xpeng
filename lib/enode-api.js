...
class EnodeAPI {
  ...
  async waitForAction(clientId, clientSecret, actionId, timeoutMs = 30000, pollIntervalMs = 2000) {
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      switch (status.state) {
        ...
        case 'FAILED':
        case 'CANCELLED':
          throw new Error(`Action ${actionId} ${status.state.toLowerCase()}: ${status.failureReason || 'No reason provided'}`);
        case 'PENDING':
