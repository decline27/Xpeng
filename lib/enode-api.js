...
class EnodeAPI {
  ...
  async generateVehicleLink(userId, clientId = null) {
    try {
      // If no client ID is specified, use the default client for new vehicles
      if (!clientId) {
        clientId = this.clientManager.getDefaultClient();
      }

      this.api.log(`Generating vehicle link using client: ${clientId}`);
  ...
  async getVehicles(clientId = null, getAllClients = true) {
    try {
      ...
      if (getAllClients) {
        // Get all clients from the client manager
        const clients = {};

        // Add primary client
        clients['primary'] = {
          clientId: this.clientManager.getClientCredentials('primary').clientId,
          clientSecret: this.clientManager.getClientCredentials('primary').clientSecret
        };

        // Add secondary client
        clients['secondary'] = {
          clientId: this.clientManager.getClientCredentials('secondary').clientId,
          clientSecret: this.clientManager.getClientCredentials('secondary').clientSecret
        };

        let allVehicles = [];

        // Get vehicles from each configured client
        for (const [clientIdentifier, clientConfig] of Object.entries(clients)) {
          if (clientConfig.clientId && clientConfig.clientSecret) {
            try {
              const clientVehicles = await this._getVehiclesForClient(clientIdentifier);

              // Tag vehicles with client info for later reference
              clientVehicles.forEach(vehicle => {
                if (vehicle.information && vehicle.information.vin) {
                  this.clientManager.setVehicleClient(vehicle.information.vin, clientIdentifier);
                }
              });

              allVehicles = [...allVehicles, ...clientVehicles];
            } catch (error) {
              this.api.error(`Error fetching vehicles from client ${clientIdentifier}:`, error);
            }
          }
        }
   ...
   */
  async getVehicleData(vehicleId, clientId = null) {
    try {
      ...
      // If no client ID is provided, try to determine which client this vehicle belongs to
      if (!clientId) {
        // First check if we have a VIN-to-client mapping
        const vehicles = await this.getVehicles();
        const vehicle = vehicles.find(v => v.id === vehicleId);

        if (vehicle && vehicle._clientId) {
          clientId = vehicle._clientId;
          this.api.log(`Using client ${clientId} for vehicle ${vehicleId} based on vehicle list`);
        } else if (vehicle && vehicle.information && vehicle.information.vin) {
          // If we have the VIN, check if it's mapped to a client
          const vin = vehicle.information.vin;
          clientId = this.clientManager.getVehicleClient(vin);
          this.api.log(`Using client ${clientId} for vehicle ${vehicleId} (VIN: ${vin}) based on mapping`);
        }
      }

      // Get token for the appropriate client
      const accessToken = await this.getAccessToken(vehicleId, clientId);
