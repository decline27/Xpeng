...
async function analyzeDuplicates(api) {
    ...
    const staleRecords = [];
    const keptByUser = new Set();
    let dupVins = 0;
    for (const recs of Object.values(byVin)) {
        if (recs.length < 2) {
            keptByUser.add(recs[0].userId); // single copy → always kept/live
            continue;
        }
        dupVins++;
        recs.sort((a, b) => lastSeenMs(b) - lastSeenMs(a)); // newest first
        keptByUser.add(recs[0].userId);
    ...
    for (const s of staleRecords) {
        ...
        const bucket = keptByUser.has(s.userId) ? risky : safe;
